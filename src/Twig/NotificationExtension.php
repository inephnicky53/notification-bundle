...
class NotificationExtension extends AbstractExtension
{
    ...
    {
        return array(
            new TwigFunction('ineph_notification_render', [$this, 'render'], ['is_safe' => ['html']]),
            new TwigFunction('ineph_notification_list', [$this, 'listNotifications'], ['is_safe' => ['html']]),
            new TwigFunction('ineph_notification_count', [$this, 'countNotifications'], ['is_safe' => ['html']]),
            new TwigFunction('ineph_notification_unseen_count', [$this, 'countUnseenNotifications'], ['is_safe' => ['html']]),
            new TwigFunction('ineph_notification_generate_path', [$this, 'generatePath'], ['is_safe' => ['html']])
        );
    }
    ...
    public function renderNotifications(NotifiableInterface $notifiable, array $options)
    {
        ...
        $template = array_key_exists('template', $options) ? $options['template'] : '@InephNotification/notification_list.html.twig';
    ...
    public function listNotifications(NotifiableInterface $notifiable, array $options)
    {
        $order = array_key_exists('order', $options) ? $options['order'] : null;
        $limit = array_key_exists('limit', $options) ? $options['limit'] : null;
        $offset = array_key_exists('offset', $options) ? $options['offset'] : null;
    ...
    {
        //dd($notifiable);
        if ($notifiable instanceof NotifiableInterface || $notifiable instanceof UserInterface) {

...
namespace Ineph\NotificationBundle\Manager;

use Doctrine\Common\Util\ClassUtils;
use Doctrine\ORM\EntityNotFoundException;
use Ineph\NotificationBundle\Entity\NotifiableEntity;
use Ineph\NotificationBundle\Entity\NotifiableNotification;
use Ineph\NotificationBundle\Entity\Notification;
use Ineph\NotificationBundle\Entity\NotificationInterface;
use Ineph\NotificationBundle\Event\NotificationEvent;
use Ineph\NotificationBundle\InephNotificationEvents;
use Ineph\NotificationBundle\NotifiableDiscovery;
 ...
 * @package Ineph\NotificationBundle\Manager
 */
class NotificationManager
{
    protected $container;
    ...
    public function __construct(Container $container, NotifiableDiscovery $discovery)
    {
        ...
        $this->notifiableRepository = $this->om->getRepository('InephNotificationBundle:NotifiableEntity');
        $this->notificationRepository = $this->om->getRepository('InephNotificationBundle:Notification');
        $this->notifiableNotificationRepository = $this->om->getRepository('InephNotificationBundle:NotifiableNotification');
    }
    ...
    {
        //$container->getParameter('ineph_notification.notification_class')
        $notificationClass = $this->container->getParameter('ineph_notification.notification_class');
        ...
        $this->dispatcher->dispatch($event, InephNotificationEvents::CREATED);
     ...
     * @throws \RuntimeException
     */
    public function addNotification($notifiables, NotificationInterface $notification, $flush = true)
    {
        foreach ($notifiables as $notifiable) {
            ...
            $this->dispatcher->dispatch($event, InephNotificationEvents::ASSIGNED);
        }
    ...
    {
        $repo = $this->om->getRepository('InephNotificationBundle:NotifiableNotification');
        foreach ($notifiables as $notifiable) {
            ...
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch($event, InephNotificationEvents::REMOVED);
            $this->flush($flush);
        }
    ...
    public function deleteNotification(NotificationInterface $notification, $flush = false)
    {
        ...
        $event = new NotificationEvent($notification);
        $this->dispatcher->dispatch($event, InephNotificationEvents::DELETED);
    }
    ...
    public function markAsSeen(NotifiableInterface $notifiable, NotificationInterface $notification, $flush = false)
    {
        ...
        if ($nn) {
            $nn->setSeen(true);
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch($event, InephNotificationEvents::SEEN);
            $this->flush($flush);
        } else {
    ...
    public function markAsUnseen(NotifiableInterface $notifiable, NotificationInterface $notification, $flush = false)
    {
        ...
        if ($nn) {
            $nn->setSeen(false);
            $event = new NotificationEvent($notification, $notifiable);
            $this->dispatcher->dispatch($event, InephNotificationEvents::UNSEEN);
            $this->flush($flush);
        } else {
            throw new EntityNotFoundException('The link between the notifiable and the notification has not been found');
        }
    ...
    public function setDate(NotificationInterface $notification, \DateTime $dateTime, $flush = false)
    {
        ...
        $event = new NotificationEvent($notification);
        $this->dispatcher->dispatch($event, InephNotificationEvents::MODIFIED);
    ...
    /**
     * Create and add notification for send to notifiables
     *
     * @param array $notifiables
     * @param string $title
     * @param string $message
     ...
     * @throws \Doctrine\ORM\OptimisticLockException
     */
    public function sendNotification(array $notifiables, string $title, string $message, string $link)
    {
        $notif = $this->createNotification($title, $message, $link);
        $this->addNotification($notifiables, $notif);
    }
}
